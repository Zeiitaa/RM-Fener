...
<template>
  <div @click="showPopup = true" class="bg-white rounded-2xl overflow-hidden shadow-lg hover:scale-105 hover:cursor-pointer transition-all duration-200 w-[80vw] max-w-[250px]
    sm:w-[35vw]
    md:w-[30vw]
    lg:w-[22.5vw]
    xl:w-[20vw]
    ...
    <div class="p-4 space-y-2">
      ...
      <div class="flex items-center justify-between mt-3">
        <div class="text-xs font-semibold">
          <span class="text-red-500">Lokasi {{ props.meja.lokasi }} /Hour</span>
        </div>
  ...
  <div v-if="showPopup" class="fixed inset-0 bg-black/40 bg-opacity-40 flex items-center justify-center z-50"
    ...
    <div class="bg-[#232528] p-6 rounded-xl shadow-xl w-[85%] h-[55%] max-w-md max-h-md">
      ...
      <span class="text-white">Lokasi {{ props.meja.lokasi }} /Hour</span>

      <form @submit.prevent="reqReservation">
        ...
        <RouterLink v-else to="/login" class="px-4 py-2 mt-10 bg-blue-500 text-white rounded-lg" style="border: 2px solid;">
          Login
        </RouterLink>
      </form>
